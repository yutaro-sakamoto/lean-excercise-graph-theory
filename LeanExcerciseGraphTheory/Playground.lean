...
import LeanExcerciseGraphTheory.PathUtil
...
lemma uv_edge (i : ZMod n) : (dgpg n t nGt1).Adj (u i) (v (i + t)) := by
  ...
  · -- i ∈ List.range n を証明
    ...
    · exact i.val_lt
    · simp
  · -- s(u i, v (i + t)) = s(u ↑i, v ↑(i + t)) を証明
    simp

lemma vu_edge (i : ZMod n) : (dgpg n t nGt1).Adj (v i) (u (i + t)) := by
  simp [dgpg, vu_ne]
  apply elem_list_to_set
  apply List.mem_append_right
  apply List.mem_append_right
...
def uv_walk (i : ZMod n) : SimpleGraph.Walk (dgpg n t nGt1) (u i) (v (i + t)) :=
  SimpleGraph.Walk.cons (uv_edge n t nGt1 i) SimpleGraph.Walk.nil
...
theorem walk_X_is_path (nGt2 : n > 2) (i : ZMod (n / 2))
  ...
  · constructor
    · intro h
      ...
      have h_snd := congrArg Prod.snd h_val
      simp at h_snd
      have one_neq_zero : (1 : ZMod n) ≠ (0 : ZMod n) := by
        intro h_contra
        have n_ne_zero : n ≠ 0 := n_neq_zero n nGt1
        have : NeZero n := ⟨n_ne_zero⟩
        exact absurd_one_eq_zero n nGt1 h_contra
...
-- 実用的な使用例
example (p1 : SimpleGraph.Path (dgpg n t nGt1) (x 0) (u 1))
        (p2 : SimpleGraph.Path (dgpg n t nGt1) (u 1) (x 2))
        (h : List.Disjoint p1.val.support.tail p2.val.support.dropLast) :
  SimpleGraph.Path (dgpg n t nGt1) (x 0) (x 2) :=
  safe_concat_paths p1 p2 h
